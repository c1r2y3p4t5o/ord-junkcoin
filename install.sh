...
git=backluv/ord-luckycoin
crate=ord
url=https://github.com/blackluv/ord-luckycoin
...
if [ -z ${tag-} ]; then
  tag=$(curl --proto =https --tlsv1.2 -sSf https://api.github.com/repos/blackluv/ord-luckycoin/releases/latest |
    grep tag_name |
    cut -d'"' -f4
  )
fi
...
if [ -z ${target-} ]; then
  ...
  case $uname_target in
    ...
    *)
      ...
      err 'Please try building from source: https://github.com/blackluv/ord-luckycoin#building'
    ;;
  esac
fi
