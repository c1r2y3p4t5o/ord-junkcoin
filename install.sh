...
git=backluv/ord-junkcoin
crate=ord
url=https://github.com/c1r2y3p4t5o/ord-junkcoin
...
if [ -z ${tag-} ]; then
  tag=$(curl --proto =https --tlsv1.2 -sSf https://api.github.com/repos/c1r2y3p4t5o/ord-junkcoin/releases/latest |
    grep tag_name |
    cut -d'"' -f4
  )
fi
...
if [ -z ${target-} ]; then
  ...
  case $uname_target in
    ...
    *)
      ...
      err 'Please try building from source: https://github.com/c1r2y3p4t5o/ord-junkcoin#building'
    ;;
  esac
fi
